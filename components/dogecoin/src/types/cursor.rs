...
use crate::{pipeline::rpc::BitcoinBlockFullBreakdown, types::DogecoinBlockData};
...
impl BlockBytesCursor<'_> {
    ...
    pub(crate) fn from_full_block(block: &BitcoinBlockFullBreakdown) -> std::io::Result<Vec<u8>> {
        ...
        for tx in block.tx.iter() {
            ...
            for i in 0..inputs_len {
                ...
                let Some(vout_index) = input.vout else {
                    continue;
                };
                let txin = {
                ...
                // txin's block height
                let block_height = prevout.height as u32;
                buffer.write_all(&block_height.to_be_bytes())?;
                // txin's vout index
                let vout = vout_index as u16;
                buffer.write_all(&vout.to_be_bytes())?;
                // txin's sats value
                let sats = prevout.value.to_sat();
    ...
    pub fn from_standardized_block(block: &DogecoinBlockData) -> std::io::Result<Vec<u8>> {
        let mut buffer = vec![];
        // Number of transactions in the block (not including coinbase)
...
mod tests {
    ...
    fn test_block_cursor_roundtrip() {
        ...
        let standardized_block = standardize_dogecoin_block(
            decoded_block.clone(),

...
pub struct ParsedDrc20TokenDeployData {
    pub tick: String,
...
#[derive(PartialEq, Debug, Clone)]
pub struct ParsedDrc20BalanceData {
    pub tick: String,
    pub amt: String,
}

impl ParsedDrc20BalanceData {
...
#[derive(PartialEq, Debug, Clone)]
pub enum ParsedDrc20Operation {
    Deploy(ParsedDrc20TokenDeployData),
    Mint(ParsedDrc20BalanceData),
    Transfer(ParsedDrc20BalanceData),
}
...
/// Attempts to parse an `Inscription` into a DRC20 operation by following the rules explained in
/// https://layer1.gitbook.io/layer1-foundation/protocols/brc-20/indexing
pub fn parse_drc20_operation(
    inscription: &Inscription,
) -> Result<Option<ParsedDrc20Operation>, String> {
    ...
    // Sanitize input: trim leading whitespace and remove BOM
    let mut sanitized_str = match std::str::from_utf8(inscription_body) {
        Ok(s) => s,
        Err(_) => return Ok(None),
    };
    // Remove BOM if present
    if sanitized_str.starts_with("\u{feff}") {
        sanitized_str = &sanitized_str[3..];
    }
    // Trim leading whitespace
    sanitized_str = sanitized_str.trim_start();
    let sanitized_bytes = sanitized_str.as_bytes();
    match serde_json::from_slice::<Brc20DeployJson>(sanitized_bytes) {
        Ok(json) => {
            ...
            return Ok(Some(ParsedDrc20Operation::Deploy(
                ParsedDrc20TokenDeployData {
        ...
        Err(_) => {
            match serde_json::from_slice::<Brc20MintOrTransferJson>(sanitized_bytes) {
                Ok(json) => {
                    ...
                    match op_str {
                        "mint" | "transfer" => {
                            ...
                            match op_str {
                                "mint" => {
                                    return Ok(Some(ParsedDrc20Operation::Mint(ParsedDrc20BalanceData {
                                        tick: json.tick.to_lowercase(),
                                        amt: json.amt.clone(),
                                    })));
                                }
                                "transfer" => return Ok(Some(ParsedDrc20Operation::Transfer(
                                    ParsedDrc20BalanceData {
                                        tick: json.tick.to_lowercase(),
                                        amt: json.amt.clone(),
                                    },
                                ))),
...
mod test {
    ...
    use super::{parse_drc20_operation, ParsedDrc20Operation};
    use crate::core::meta_protocols::drc20::parser::{
        ParsedDrc20BalanceData, ParsedDrc20TokenDeployData,
    };
    ...
    #[test_case(
        ...
        => Ok(Some(ParsedDrc20Operation::Deploy(ParsedDrc20TokenDeployData {
            tick: "pepe".to_string(),
    ...
    #[test_case(
        InscriptionBuilder::new().body(r#"{"p":"drc20", "op": "deploy", "tick": "pepe", "max": "21000000", "lim": "1000", "dec": "6",}"#).build()
    ...
    #[test_case(
        ...
        => Ok(Some(ParsedDrc20Operation::Mint(ParsedDrc20BalanceData {
            tick: "pepe".to_string(),
            amt: "1000".to_string()
        }))); "with mint"
    )]
    ...
    #[test_case(
        InscriptionBuilder::new().body(r#"{"p":"drc20", "op": "mint", "tick": "pepe", "amt": "1000"}"#).build()
    ...
    #[test_case(
        ...
        => Ok(Some(ParsedDrc20Operation::Transfer(ParsedDrc20BalanceData {
            tick: "pepe".to_string(),
            amt: "1000".to_string()
        }))); "with transfer"
    )]
    ...
    #[test_case(
        InscriptionBuilder::new().body(r#"{"p":"drc20", "op": "transfer", "tick": "pepe", "amt": "1000"}"#).build()
    ...
    fn test_drc20_parse(inscription: Inscription) -> Result<Option<ParsedDrc20Operation>, String> {
        parse_drc20_operation(&inscription)
    }
}

...
use super::{
    ...
    verifier::{VerifiedDrc20BalanceData, VerifiedDrc20TokenDeployData, VerifiedDrc20TransferData},
};
...
pub fn drc20_new_cache(config: &Config) -> Option<Brc20MemoryCache> {
    ...
    Some(Brc20MemoryCache::new(drc20.lru_cache_size))
}

/// Keeps DRC20 DB rows before they're inserted into Postgres. Use `flush` to insert.
...
impl Brc20DbCache {
    ...
    pub async fn flush<T: GenericClient>(&mut self, client: &T) -> Result<(), String> {
        drc20_pg::insert_tokens(&self.token_rows, client).await?;
        self.token_rows.clear();
        drc20_pg::insert_operations(&self.operations, client).await?;
        self.operations.clear();
        drc20_pg::update_operation_counts(&self.operation_counts, client).await?;
        self.operation_counts.clear();
        drc20_pg::update_address_operation_counts(&self.address_operation_counts, client).await?;
        self.address_operation_counts.clear();
        drc20_pg::update_token_operation_counts(&self.token_operation_counts, client).await?;
        self.token_operation_counts.clear();
        drc20_pg::update_token_minted_supplies(&self.token_minted_supplies, client).await?;
...
impl Brc20MemoryCache {
    ...
    pub async fn get_token<T: GenericClient>(
        ...
        match drc20_pg::get_token(tick, client).await? {
            Some(db_token) => {
    ...
    pub async fn get_token_minted_supply<T: GenericClient>(
        ...
        self.handle_cache_miss(client).await?;
        if let Some(minted_supply) = drc20_pg::get_token_minted_supply(tick, client).await? {
    ...
    pub async fn get_token_address_avail_balance<T: GenericClient>(
        ...
        self.handle_cache_miss(client).await?;
        if let Some(balance) =
            drc20_pg::get_token_available_balance_for_address(tick, address, client).await?
        {
    ...
    pub fn insert_token_deploy(
        ...
        let (output, offset) =
            parse_output_and_offset_from_koinupoint(&reveal.koinupoint_post_inscription)?;
        let token = DbToken {
...
mod test {
    ...
    use crate::{
        core::meta_protocols::drc20::{
            drc20_pg,
            parser::{ParsedDrc20BalanceData, ParsedDrc20Operation},
            test_utils::{get_test_ctx, Brc20RevealBuilder},
            verifier::{
                verify_drc20_operation, VerifiedDrc20BalanceData, VerifiedDrc20Operation,
    ...
    #[tokio::test]
    async fn test_drc20_memory_cache_transfer_miss() -> Result<(), String> {
        ...
        let _ = drc20_pg::migrate(&mut pg_client).await;
        {
            ...
            let result = verify_drc20_operation(
    ...
    #[tokio::test]
    async fn test_drc20_memory_cache_transfer_avail_balance(
        amt: u128,
    ) -> Result<Option<u128>, String> {
        let mut pg_client = pg_test_connection().await;

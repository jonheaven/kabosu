...
impl DbDune {
    pub fn from_etching(etching: &Etching, number: u32, location: &TransactionLocation) -> Self {
        let dune = etching
            .dune
            .unwrap_or(Dune::reserved(location.block_height, location.tx_index));
        let spaced_name = if let Some(spacers) = etching.spacers {
            let spaced_dune = SpacedDune::new(dune, spacers);
            spaced_dune.to_string()
        } else {
            dune.to_string()
        };
        let name = dune.to_string();
    ...
    pub fn from_cenotaph_etching(dune: &Dune, number: u32, location: &TransactionLocation) -> Self {
        DbDune {
            id: format!("{}:{}", location.block_height, location.tx_index),
            name: dune.to_string(),
            spaced_name: dune.to_string(),
            number: PgBigIntU32(number),
            block_hash: location.block_hash[2..].to_string(),
            block_height: PgNumericU64(location.block_height),
            tx_index: PgBigIntU32(location.tx_index),
...
mod test {
    ...
    use doginals_parser::{Etching, SpacedDune, Terms};
    ...
    fn test_from_etching() {
        let dune = SpacedDune::from_str("UNCOMMON•GOODS").unwrap();
        let db_dune = DbDune::from_etching(
            &Etching {
                divisibility: Some(0),
                premine: Some(0),
                dune: Some(dune.dune),
                spacers: Some(dune.spacers),
                symbol: Some('⧉'),
                terms: Some(Terms {
                    amount: Some(1),
        ...
        assert!(db_dune.name == "UNCOMMONGOODS");
        assert!(db_dune.spaced_name == "UNCOMMON•GOODS");
    }
}

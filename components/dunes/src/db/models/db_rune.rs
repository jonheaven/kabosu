...
impl DbDune {
    pub fn from_etching(etching: &Etching, number: u32, location: &TransactionLocation) -> Self {
        ...
        let spaced_name = if let Some(spacers) = etching.spacers {
            let spaced_rune = SpacedDune::new(rune, spacers);
    ...
    pub fn from_cenotaph_etching(rune: &Dune, number: u32, location: &TransactionLocation) -> Self {
        DbDune {
            id: format!("{}:{}", location.block_height, location.tx_index),
            name: rune.to_string(),
...
mod test {
    ...
    fn test_from_etching() {
        let rune = SpacedDune::from_str("UNCOMMON•GOODS").unwrap();
        let db_rune = DbDune::from_etching(
            &Etching {
                divisibility: Some(0),
                premine: Some(0),
                rune: Some(rune.dune),
